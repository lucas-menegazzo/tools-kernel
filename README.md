...
**Why it matters:** for a company spending US$ 250K/year on a low-code platform and planning ten more apps, the build-or-buy question hinges on whether the marginal cost of each new app is lower than another bought license. This prototype shows that once the kernel exists, each new app gets cheaper: the first app cost 50 ACUs, the fourth 30 ACUs, and the fifth 25 ACUs. It also makes regulated controls unavoidable—audit, RLS and approval are enforced in the database, not in UI properties that can be bypassed or copied wrong.
